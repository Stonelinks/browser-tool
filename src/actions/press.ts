...
export async function browserPress(input: PressInput): Promise<ActionResult<PressResult>> {
  ...
  return withSession(input.taskId, async (session) => {
    try {
      ...
      await session.page.waitForLoadState("domcontentloaded", { timeout: 2000 }).catch(() => undefined);
      return {
        success: true,
        pressed: input.key,
